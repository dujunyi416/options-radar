...
def rank(prompt: str, attempts: int = 2) -> dict:
    ...
    for backend in backends:
        for i in range(attempts):
            try:
                print(
                    f"[info] rank via {backend} attempt {i + 1}/{attempts}",
                    file=sys.stderr,
                )
                return extract_json(_call_llm(prompt, backend))
            except (RuntimeError, ValueError, json.JSONDecodeError) as exc:
                last_exc = exc
                print(
                    f"[warn] {backend} attempt {i + 1}/{attempts} failed: {exc}",
                    file=sys.stderr,
                )
        print(f"[warn] {backend} exhausted, trying next backend", file=sys.stderr)
    raise RankFailure(f"all backends failed: {last_exc}") from last_exc
...
def _classify_failure(reason: str) -> str:
    ...
    if "timeout" in r or "timed out" in r:
        return "LLM 调用超时 (API 慢或网络异常), 无需立即处理, 下周 cron 再看。"
